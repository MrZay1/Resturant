...
1. Add the slug to `site/data/links.json` as in Section 9A, deploy, and confirm `https://tblnt.co/r/<slug>?s=card` redirects on your phone. Slugs may contain only lowercase letters, digits, hyphen and underscore; `site/lib/links.ts` strips everything else, and `links.json` is imported at build time so a new slug is live only after the next deploy.
2. Write the chip with NFC Tools: Write, Add a record, URL, paste `https://tblnt.co/r/<slug>?s=card`, Write, hold the top edge of the phone on the card until it confirms (https://www.wakdev.com/en/apps/nfc-tools-ios.html). The URL is about 35 characters; NTAG213 holds roughly 130 (https://www.nxp.com/docs/en/data-sheet/NTAG213_215_216.pdf).
...
| One-page leave-behind | 10 | Not yet in the repo. Contents: three bullets on what it is, the pricing table ($15 per card, $50 a month with 10 replacements, starter kit $200 then $50), the founding offer (baseline report in 48 hours, half-price report for 3 months, cancel anytime), four sourced stats (97 percent read reviews, 74 percent look for reviews from the last three months, 83 percent of those asked leave one, 60 vs 29 positive vs negative, all https://www.brightlocal.com/research/local-consumer-review-survey/), your phone, email and Tablenote's physical postal address, a QR to tablenote.co/demo. Suggested: add a `/print/leave-behind` page next to the existing print pages and extend `export-print.mjs` |
...
- [ ] Deploy. Test `https://tblnt.co/r/<slug>?s=card` on an iPhone and an Android: a signed-out phone must land on `accounts.google.com/ServiceLogin?continue=...writereview?placeid=<same ID>`.
...
- [ ] Cards ship about 10 business days after proof approval, US shipping included. Encode `https://tblnt.co/r/<slug>?s=card`, test, then lock read-only.
